...
def import_data():
	r = \
		requests.get('https://github.com/shubham0204/Dataset_Archives/blob/master/chatbot_nlp.zip?raw=true'
					 )
...
padded_questions = \
	preprocessing.sequence.pad_sequences(tokenized_questions,
...
padded_answers = \
	preprocessing.sequence.pad_sequences(tokenized_answers,
...
model.fit([encoder_input_data, decoder_input_data],
		  decoder_output_data, batch_size=64, epochs=300)
...
for _ in range(10):
	states_values = \
		enc_model.predict(str_to_tokens(input('Enter question : ')))
